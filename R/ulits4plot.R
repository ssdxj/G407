...
gglm <- function(fit, smooth = 'lm') {
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) +
    ...
    labs(title = paste("Adj R2 = ", sprintf('%.3f', summary(fit)$adj.r.squared),
...
train_obsvspred_plot <- function(train_obj){
  ...
  if(is.null(testDf)){
    ...
    gof <- train_obj$add_gof %>%
      mutate(label = sprintf('Rsquared=%.2f\nRMSE=%.2f\nMAE=%.2f',
    ...
    gof <- train_obj$add_gof %>%
      mutate(Calibration = sprintf('Rsquared=%.2f\nRMSE=%.2f\nMAE=%.2f',
                                   TrainRsquared, TrainRMSE, TrainMAE)) %>%
      mutate(Validation = sprintf('Rsquared=%.2f\nRMSE=%.2f\nMAE=%.2f',
...
trains_obsVsPred_plot <- function(train_objs, ...){
  ...
  p1 <- ggplot(df1, aes(x = obs, y = pred)) +
    ...
    geom_text(aes(label = Clabel), size = 3,
  ...
  if(nrow(df2) > 0){
    gof_df <- map_df(train_objs, 'add_gof', .id = 'tag') %>%
      mutate(Vlabel = sprintf('Rsquared=%.2f\nRMSE=%.2f',
    ...
    p2 <- ggplot(df2, aes(x = obs, y = pred)) +
      geom_point(aes_string(...)) +
      geom_text(aes(label = Vlabel), size = 3,
                x = -Inf, y = Inf, hjust = -0.1, vjust = 1.2,
                parse = FALSE, data = gof_df) +
      geom_abline(intercept = 0, slope = 1, color = "gray50") +
      facet_wrap(~tag, scales = 'fixed', labeller = label_parsed) +
...
trains_gof_dotplot <- function(input, tagName = 'method'){
  input %>%
    gather(matric, value, contains('RMSE'), contains('MAE'),
           contains('Rsquared')) %>%
...
train_residual_plot <- function(train_obj){
  ...
  if(is.null(testDf)){
    ...
    df1 <- rbind(
      data.frame(x = 1:nrow(trainDf), y = trainDf$pred - trainDf$obs,
                 key1 = 'Calibration',  key2 = 'ID'),
      data.frame(x = 1:nrow(testDf), y = testDf$pred - testDf$obs,
                 key1 = 'Validation', key2 = 'ID')
    )

    df2 <- rbind(
      data.frame(x = trainDf$obs, y = trainDf$pred - trainDf$obs,
                 key1 = 'Calibration',  key2 = 'Obs'),
      data.frame(x = testDf$obs, y = testDf$pred - testDf$obs,
